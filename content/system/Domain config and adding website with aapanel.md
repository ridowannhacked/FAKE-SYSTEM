...
This guide walks you through connecting your custom domain (`aliridowan.com`) to Cloudflare, pointing DNS records to your VPS (`157.173.115.123`), hosting your React production build on aaPanel, and configuring end-to-end SSL/TLS encryption.
...
User (Browser) 
      ...
      ▼
Cloudflare Edge Proxy (DNS, CDN, WAF)
      │  HTTPS / Encrypted Origin Pull
      ▼
VPS Server (157.173.115.123: Nginx / aaPanel)
      │  Serves static bundle + SPA fallback
      ▼
React Build Output (/www/wwwroot/aliridowan.com/index.html)
```
...
2. **Add Domain:**
   ...
   - Enter `aliridowan.com` and select the **Free Plan**.
3. **Scan Records:**
...
1. In the **Review your DNS records** screen:
   - **Root Domain Record:**
     - **Type:** `A`
     - **Name:** `@` (or `aliridowan.com`)
     - **IPv4 Address:** `157.173.115.123` (Your VPS Public IP)
     - **Proxy status:** **Proxied** (Orange cloud enabled)
     - **TTL:** `Auto`
   - **WWW Subdomain Record:**
     - **Type:** `CNAME`
     - **Name:** `www`
     - **Target:** `aliridowan.com`
     - **Proxy status:** **Proxied** (Orange cloud enabled)
     - **TTL:** `Auto`
...
1. Log in to the registrar where you purchased `aliridowan.com` (e.g., Namecheap, Hostinger, GoDaddy, Porkbun).
2. Find the **DNS Management** or **Nameserver** section for `aliridowan.com`.
...
1. **Access aaPanel:**
   - Navigate to your aaPanel panel: `http://157.173.115.123:27570/<security_entrance>`
2. **Add Website:**
   ...
   - **Description:** `aliridowan_portfolio`
   - **Root Directory:** Default is `/www/wwwroot/aliridowan.com`
...
1. In aaPanel, navigate to **Files** > `/www/wwwroot/aliridowan.com`.
2. Delete the default files (`index.html`, `404.html`, `.htaccess`).
3. Click **Upload** and upload your production files:
   - Ensure `index.html` resides directly in `/www/wwwroot/aliridowan.com/index.html`.
...
1. **In Cloudflare Dashboard:**
   ...
   - Keep default hostnames (`aliridowan.com`, `*.aliridowan.com`) and 15-year validity.
   - Click **Create**.
...
- [ ] **Proxy Verification:**
  Pinging `aliridowan.com` returns a Cloudflare anycast IP (e.g. `104.x.x.x` or `172.x.x.x`), successfully masking your origin VPS IP (`157.173.115.123`).
- [ ] **SPA Route Refresh:**
  Navigate to an internal route (e.g., `https://aliridowan.com/about`) and reload to verify client routing handles the path seamlessly.
- [ ] **SSL Grade:**
  Confirm green padlock status on both `https://aliridowan.com` and `https://www.aliridowan.com`.
...
| Issue | Cause | Fix |
|---|---|---|
| **Old host page displays** | Cloudflare `A` record points to old provider IP or local DNS cache. | Update Cloudflare `A` record to `157.173.115.123`, flush DNS (`ipconfig /flushdns` or browser cache). |
